...
from utils import load_data, generate_sampled_graph_and_labels, build_test_graph, calc_mrr, score_loss, reg_loss
...
def train(train_triplets, model, use_cuda, batch_size, split_size, negative_sample, reg_ratio, num_entities,
    ...
    entity_embedding = model.encode(train_data.entity, train_data.edge_index, train_data.edge_type, train_data.edge_norm)

    score = model.decode((train_data.samples[:, 0], train_data.samples[:, 2]), train_data.samples[:, 1])

    loss = score_loss(score, train_data.labels) + reg_ratio * reg_loss(entity_embedding, model.decoder.rel_emb)
...
def main(args):
    ...
    model = GAE(
        GNNEncoder(len(entity2id), input_channels=INPUT_CHANNELS, hidden_channels=HIDDEN_CHANNELS,
                   output_channels=OUTPUT_CHANNELS, num_relations=len(relation2id), gnn_model=args.encoder),
        DistMultDecoder(len(relation2id), input_channels=INPUT_CHANNELS),
    )
